...
class Service_types extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->load->model('Service_type_model', 'svt');
    }

    public function index()
    {
        $this->layout->view('service_types/index_view');
    }

    public function get_list(){

        $result = $this->svt->get_list();
        if($result){
    ...
    public function save(){
        ...
        if(empty($data['name'])){
            ...
            if(empty($data['id'])){
                $duplicated = $this->svt->check_duplicate($data['name']);
                if($duplicated){
                    $json = '{"success": false, "msg": "Name duplicated"}';
                }else{
                    $result = $this->svt->save($data['name']);
                    if($result){
