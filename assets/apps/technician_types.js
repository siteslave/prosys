$(function(){
    var technician_types = {};

    technician_types.modal = {
        show_new: function(){
    ...
    technician_types.ajax = {
        save: function(data, cb){
            var url = '/technician_types/save',
                params = {
                    data: data
                };
        ...
        remove: function(id, cb){
            var url = '/technician_types/remove',
                params = {
                    id: id
                };
        ...
        get_list: function(start, stop, cb){
            var url = '/technician_types/get_list',
                params = {
                	start: start, 
                	stop: stop
                };
        ...
        get_list_total: function(cb){
            var url = '/technician_types/get_list_total',
                params = {};
        ...
        search: function(query, cb){
            var url = '/technician_types/search',
                params = {
                    query: query
                };
	...
	technician_types.set_list = function(data){
	...
	technician_types.get_list = function(){
	    $('#main_paging').fadeIn('slow');
		technician_types.ajax.get_list_total(function(err, data){
	        if(err){
	            App.alert(err);
	        }else{
	            $('#main_paging > ul').paging(data.total, {
	                ...
	                onSelect: function(page){
	                    ...
	                    technician_types.ajax.get_list(this.slice[0], this.slice[1], function(err, data){
	                    	$('#tbl_list tbody').empty();
	                        if(err){
	                            App.alert(err);
	                        }else{
	                            technician_types.set_list(data);
	                        }
    ...
    $('#btn_new').click(function(){
        technician_types.modal.show_new();
    });
    ...
    $('#btn_save').click(function(){
        ...
        if(!items.name){
            ...
            technician_types.ajax.save(items, function(err){
                if(err){
                    App.alert(err);
                }else{
                    $('#txt_name').val('');
    ...
    $('a[data-name="remove"]').live('click', function(){
        ...
        if(confirm('คุณต้องการลบรายการนี้ใช่หรือไม่?')){
            technician_types.ajax.remove(id, function(err){
                if(err){
                    App.alert(err);
                }else{
                    //remove row
    ...
    $('#btn_search').click(function(){
        ...
        if(query && query.length > 2){
            //do search
            technician_types.ajax.search(query, function(err, data){
                if(err){
                    App.alert(err);
                    $('#tbl_list tbody').append(
                        '<tr><td colspan="2">ไม่พบรายการ</td></tr>'
