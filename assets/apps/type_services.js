$(function(){
    ...
    type_services.ajax = {
        ...
        get_list: function(start, stop, cb){
            var url = '/type_services/get_list',
                params = {
                	start: start, 
                	stop: stop
                };
        ...
        get_list_total: function(cb){
            var url = '/type_services/get_list_total',
                params = {};
	...
	type_services.set_list = function(data){
	...
	type_services.get_list = function(){
	    $('#main_paging').fadeIn('slow');
		type_services.ajax.get_list_total(function(err, data){
	        if(err){
	            App.alert(err);
	        }else{
	            $('#main_paging > ul').paging(data.total, {
	                ...
	                onSelect: function(page){
	                    ...
	                    type_services.ajax.get_list(this.slice[0], this.slice[1], function(err, data){
	                    	$('#tbl_list tbody').empty();
	                        if(err){
	                            App.alert(err);
	                        }else{
	                            type_services.set_list(data);
	                        }
    ...
    $('#btn_save').click(function(){
        ...
        if(!items.name){
            ...
            type_services.ajax.save(items, function(err){
                if(err){
                    App.alert(err);
                }else{
                    $('#txt_name').val('');
    ...
    $('a[data-name="remove"]').live('click', function(){
        ...
        if(confirm('คุณต้องการลบรายการนี้ใช่หรือไม่?')){
            type_services.ajax.remove(id, function(err){
                if(err){
                    App.alert(err);
                }else{
                    //remove row
    ...
    $('#btn_search').click(function(){
        ...
        if(query && query.length > 2){
            //do search
            type_services.ajax.search(query, function(err, data){
                if(err){
                    App.alert(err);
                    $('#tbl_list tbody').append(
                        '<tr><td colspan="2">ไม่พบรายการ</td></tr>'
